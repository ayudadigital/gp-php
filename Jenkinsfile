...
cfg = jplConfig('gp-php', 'bash', '', [email: env.CI_NOTIFY_EMAIL_TARGETS])
...
def buildAndPublishDockerImage(nextReleaseNumber = "") {
    ...
    docker.withRegistry("", 'docker-token') {
        def customImage = docker.build("${env.DOCKER_ORGANIZATION}/${cfg.projectName}:${nextReleaseNumber}", "--pull --no-cache webapp")
        customImage.push()
